...
class Service(db.Model):
    ...
    doctor_id = db.Column(db.Integer, db.ForeignKey('doctor.id'))
    doctor = db.relationship('Doctor', backref='services')
...
def index():
    ...
    if query:
        results = {
            ...
            'services': Service.query.join(Doctor).filter(
                (Service.name.contains(query)) | (Doctor.name.contains(query))
            ).all()
        }
...
def add_patient():
    ...
    p = Patient(name=request.form['name'], age=int(age) if age else None)
    db.session.add(p)
    db.session.commit()
